...
def dp(rooms, corridor):
    if all([value == (key, key, key, key) for key, value in rooms]):
        return 0
    ...
    for i in range(7):
        ...
        if rooms[aphid] == () or all([aphid == x for x in rooms[corridor[i]]]):
            ...
            if i > ROOMINDEX[aphid] and all([x == "." for x in corridor[ROOMINDEX[aphid]+1:i]]):
                numberOfSteps = 5 - len(rooms[aphid]) + (i-1-ROOMINDEX[aphid]) * 2
                if i == 6:
                    numberOfSteps -= 1
                ...
                numberOfSteps = 5 - len(rooms[aphid]) + (ROOMINDEX[aphid]-i) * 2
                if i == 0:
                    numberOfSteps -= 1
    ...
    for key, value in rooms.items():
        ...
        cost = STEPCOST[aphid]*(6 - len(value))
